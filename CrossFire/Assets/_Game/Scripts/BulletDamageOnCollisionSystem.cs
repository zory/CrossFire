...
namespace CrossFire.Bullets
{
	...
	[UpdateBefore(typeof(CrossFire.Bullets.BulletDestroyOnCollisionSystem))]
	public partial struct BulletDamageOnCollisionSystem : ISystem
	{
		...
		{
			if (entityManager.HasComponent<BulletOwner>(possibleBulletEntity))
			{
				BulletOwner bulletOwner =
					entityManager.GetComponentData<BulletOwner>(possibleBulletEntity);
